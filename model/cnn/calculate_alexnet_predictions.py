...
if __name__ == "__main__":
    ...
    df = pd.DataFrame(index=np.arange(0, len(objects) * len(comparisons['o1']) * LAYER_COUNT),
                      columns=['Target', 'Comparison', 'Layer', 'Distance', 'DistanceInverted'], dtype=float)
    ...
    for obj in objects:
        ...
        for comparison in comparisons[obj]:
            ...
            comparison_net_outputs, _ = calculate_alexnet_prediction(net, transformer, labels, comparison_img)

            inverted_comparison_file = "{0:s}_inverted/{1:s}.png".format(stimuli_folder, comparison)
            inverted_comparison_img = misc.imread(inverted_comparison_file)
            inverted_comparison_img = preprocess_image(inverted_comparison_img)

            inverted_comparison_net_outputs, _ = calculate_alexnet_prediction(net, transformer, labels,
                                                                              inverted_comparison_img)
            ...
            for layer in target_net_outputs.keys():
                ...
                inverted_mse = np.mean(np.square(target_net_outputs[layer] - inverted_comparison_net_outputs[layer]))
                df.loc[i] = [obj, comparison, "AlexNet_" + layer, mse, inverted_mse]
                i += 1
        print

    # calculate model predictions
    predictions = psql.sqldf("select d1.Target as Target, d1.Comparison as Comparison1, d2.Comparison as Comparison2, "
                             "d1.Layer as Layer, d1.Distance<d2.Distance as AlexNet_Prediction "
